...
class OdometryTank : public OdometryBase {
  public:
     ...
     * @param config the specifications that supply the odometry with descriptions of the robot. See robot_specs_t for
     * what is contained
     * @param imu The robot's inertial sensor. If not included, rotation is calculated from the encoders.
     * @param is_async If true, position will be updated in the background continuously. If false, the programmer will
     * have to manually call update().
     */
    OdometryTank(
      vex::motor_group &left_side, vex::motor_group &right_side, robot_specs_t &config, vex::inertial *imu = NULL,
      bool is_async = true
    );
    ...
    OdometryTank(
      CustomEncoder &left_custom_enc, CustomEncoder &right_custom_enc, robot_specs_t &config, vex::inertial *imu = NULL,
      bool is_async = true
    );
    ...
    OdometryTank(
      vex::encoder &left_vex_enc, vex::encoder &right_vex_enc, robot_specs_t &config, vex::inertial *imu = NULL,
      bool is_async = true
    );
